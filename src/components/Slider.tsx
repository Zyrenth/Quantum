...
type Props = VariantProps<typeof sliderWrapper> & VariantProps<typeof sliderDot> & VariantProps<typeof sliderRounding>;
interface SliderProps extends Omit<InputHTMLAttributes<HTMLInputElement>, 'size'>, Props {
    /**
...
const sliderBar = cva(['transition-colors ease-out duration-[150ms]'], {
    ...
    compoundVariants: [
        ...
        {
            appearance: 'glossy',
            className: 'bg-gradient-to-b from-white/5 to-black/10 dark:from-white/10 dark:to-black/5',
        },
        {
            appearance: 'normal',
            filled: true,
...
const Slider = React.forwardRef<HTMLInputElement, SliderProps>(function Slider(
    { className, appearance, size, rounding, disabled, onInput, value, max, onUpdate, updateTimeout = 250, ...props },
    ref,
) {
    ...
    return (
        ...
        >
            <input
                ...
                {...props}
            />
            <div aria-hidden="true" className={cn(sliderBarWrapperClass, roundingClass)}>
                <div
                    className={cn(sliderBarFilledClass, roundingClass)}
                    style={{
                        width: bWidth ? (bWidth * percentage) / 100 + 'px' : '0%',
                    }}
                ></div>
                <div
                    className={cn(sliderBarClass, roundingClass)}
                    style={{
                        width: bWidth ? bWidth - (bWidth * percentage) / 100 + 'px' : '100%',
                    }}
                ></div>
            </div>
            <div
