...
const AvatarGroup = React.forwardRef<HTMLDivElement, AvatarGroupProps>(function AvatarGroup(
    ...
    return (
        <div ref={ref} aria-label={ariaLabel} className={cn(avatarGroupClass, className)} {...props}>
            {displayedChildren}
            {!!reachedMax && (
                <Avatar
                    fallback={reachedMax ? `+${React.Children.count(children) - maxDisplayed}` : undefined}
