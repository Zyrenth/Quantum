...
const Select = React.forwardRef<HTMLSelectElement, SelectProps>(function Select(
    ...
    const deselectItem = useCallback(
        (id: string) => {
            checkmarkStrokeRefs.current[id]!.offset =
                checkmarkStrokeRefs.current[id]!.length;
                    ...
                    const icon = (
                        <CheckIcon
                            ref={(node) => {
                                ...
                                const strokeLength = node
                                    ?.getElementsByTagName('path')?.[0]
                                    ?.getTotalLength();

                                checkmarkStrokeRefs.current[option.id]!.length =
                                    strokeLength;
                                checkmarkStrokeRefs.current[option.id]!.offset =
                        ...
                        action: () => {
                            ...
                            setTimeout(() => {
                                if (
                                    ...
                                    defaultRef.current.dispatchEvent(
    ...
    const base = (
        <div className={cn(selectWrapperClass, className)}>
            ...
            <Button
                ...
                disabled={disabled}>
                <span className="flex flex-row gap-2.5 w-full truncate">
                    {selectedAmount}
                </span>
            </Button>
        </div>
