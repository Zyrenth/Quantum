        ...
        compoundVariants: [
            {
                ...
                layout: 'blocky',
                className:
                    'text-black dark:text-white hover:enabled:bg-white/10 dark:hover:enabled:bg-black/10',
            },
            {
                selected: true,
                mounted: false,
            ...
            },
            {
                ...
                layout: 'flat',
                className:
                    'text-black/75 dark:text-white/75 hover:enabled:text-black dark:hover:enabled:text-white',
            },
        ],
        defaultVariants: {
            size: 'md',
...
const pill = cva(
    ['h-full top-0 left-0 bottom-0 transition-all ease-out duration-[150ms]'],
    {
        variants: {
            /**
...
const pillWrapper = cva(
    [
        'absolute w-full h-full top-0 left-0 bottom-0 right-0 pointer-events-none',
    ],
    {
        variants: {
            /**
...
const Tab = React.forwardRef<HTMLDivElement, TabProps>(function Tab(
    ...
    useEffect(() => {
        ...
        if (itemRef?.node)
            setCurrentItemWidth(itemRef.node.getBoundingClientRect().width);
        ...
        setCurrentOverallWidth(
            itemRefs
                ...
                .reduce(
                    (totalWidth, item) =>
                        totalWidth +
                        (item.node?.getBoundingClientRect()?.width ?? 0),
                    0,
                ),
        );
    ...
    useEffect(() => {
        ...
        if (itemRef?.node)
            setHoveredItemWidth(itemRef.node.getBoundingClientRect().width);
    ...
    useEffect(() => {
        const handleKeyDown = (event: KeyboardEvent) => {
            ...
            if (
                ...
                const focusedIndex = itemRefs.findIndex(
                    (item) => item.node === document.activeElement,
                );
        ...
        <div
            ...
            onPointerMove={(event) => {
                ...
                const index = itemRefs.findIndex(
                    (item) =>
                        item.node === (event.target as Node) ||
                        item.node?.contains(event.target as Node),
                );
            ...
            <div
                aria-hidden="true"
                className={cn(tabOverlayClass, pillWrapper({ layout }))}>
                {isMounted && (
                    <div
                        className={cn(
            ...
            <div
                ...
                className={cn(
                    tabOverlayClass,
                    pillWrapper({
                        layout: `overlay_${layout}` as `overlay_${'blocky' | 'flat'}`,
                    }),
                )}>
                {isMounted && (
                    <div
                        className={cn(
                            ...
                            pill({
                                ...
                                hidden:
                                    ...
                                    tabs.find(
                                        (tab) =>
                                            tab.id === tabs[hoveredTabIndex].id,
                                    )?.disabled,
                            }),
                            'duration-[150ms]',
                        )}
                        style={{
                            ...
                            transitionProperty: isInitialHover
                                ? 'opacity'
