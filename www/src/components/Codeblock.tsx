...
const codeblockHeaderIcon = cva(
    [
        'h-5 w-5 text-black/75 dark:text-white/75 transition-all ease-out duration-[150ms]',
    ],
    {
        variants: {
            /**
...
const codeblockTextarea = cva(
    [
        'w-full items-center gap-2.5 bg-black/10 dark:bg-white/10 overflow-x-auto',
    ],
    {
        variants: {
            /**
...
const Codeblock = React.forwardRef<HTMLDivElement, CodeblockProps>(
    function Codeblock(
        ...
        const ariaLabel =
            props['aria-label'] ||
            (!hideHeader ? ids.create('filename') : 'Codeblock');
        ...
        const lineDiff = (lineNumber: number) => {
            ...
            if (highlightedLines.includes(lineNumber))
                ...
                highlighter = highlightColors.added;
            else if (removedLines.includes(lineNumber))
                highlighter = highlightColors.removed;
            ...
            <>
                {allowCopy && (
                    <Button
                        ...
                        <ClipboardIcon
                            className={cn(codeblockHeaderIconClass)}
                        />
                    </Button>
                )}
                {allowDownload && (
                    <Button
                        ...
                        className="p-1"
                        onClick={() =>
                            downloadTextFile(code, filename ?? 'download.txt')
                        }>
                        <ArrowDownTrayIcon
                            className={cn(codeblockHeaderIconClass)}
                        />
        ...
        return (
            <div
                ...
                {!hideHeader ? (
                    ...
                    <div className="absolute flex flex-row gap-2.5 items-center right-0 p-2">
                        {buttons}
                    </div>
                )}
                {(disableHighlighting || !isLanguageLoaded
                    ...
                    <div
                        key={index}
                        className={cn(
                            codeblockTextarea({ size, isDark: index === 0 }),
                        )}>
                        <Highlight
                            theme={theme}
                            code={code}
