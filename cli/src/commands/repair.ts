...
class Repair {
    ...
    build() {
        return new Command()
            .name('repair')
            .description('Repair an installed component.')
            ...
            .argument('[components...]', 'The local names of the components to repair.')
            .action((...args: any[]) => {
                // @ts-ignore
                this.run(...args);
            });
    ...
    async run(inputComponents: string[], options: {
        ...
        if (inputComponents && inputComponents.length > 0 && [...inputComponents].every((component) => !Object.keys(this.config?.components ?? {}).includes(component))) {
            const badComponents = [...inputComponents].filter((component) => !Object.keys(this.config?.components ?? {}).includes(component));

            inputComponents = inputComponents.filter((component) => !badComponents.includes(component));

            warnTask({
                text: badComponents.join(', ') + ' components are not installed. Excluding them from the repair...',
            });
        }
        ...
        if (this.config.components && Object.entries(this.config.components).length === 0) {
            failTask({
        ...
        const components = inputComponents && inputComponents.length > 0 ? { value: inputComponents } : await prompts({
            type: 'multiselect',
            name: 'value',
            message: 'Select components to repair:',
            choices: Object.entries(this.config?.components ?? {}).map(([name, component]) => {
                const id = this.project.getComponentId(name);
                const version = remotes?.[component?.remote]?.components?.[id]?.version ?? '0.0.0';
                const isUpToDate = this.project.isComponentUpToDate(name, version);
                const isEnabled = remotes[component.remote]?.enabled;

                return {
                    title: `${name}\x1b[0m \x1b[90mv${component.version}${remotes[component?.remote] ? (!isUpToDate ? ' \x1b[31m✖ Update component first' : '') : ''}${remotes[component.remote] && !isEnabled ? ' \x1b[31m✖ Remote is disabled' : ''}${!remotes[component.remote] ? ' \x1b[31m✖ Invalid remote' : ''}\x1b[0m`,
                    value: name,
        ...
        await new Components(this.config, this.debug, this.noInput, this.fileNameOverride, this.forceOverride, this.skipPackage, []).repair(selectedComponents);
    }
...
export default new Repair().build();
