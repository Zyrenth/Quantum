...
class Uninstall {
    ...
    build() {
        return new Command()
            ...
            .option('-sp, --skip-package', 'Skip uninstlling the packages.')
            .option('-kd, --keep-dependencies <dependencies>', 'The local names of the dependencies to keep, separated by commas.')
            .argument('[components...]', 'The local names of the components to uninstall.')
            .action((...args: any[]) => {
                // @ts-ignore
                this.run(...args);
            });
    ...
    async run(inputComponents: string[], options: {
        ...
        this.dependenciesToKeep = options.keepDependencies ? options.keepDependencies.split(',') : [];
        ...
        if (options.yes && this.dependenciesToKeep.length === 0) {
            failTask({
                text: 'No dependencies provided to keep with the `--yes` flag.',
            });
        ...
        if (inputComponents && inputComponents.length > 0 && [...inputComponents].every((component) => this.config?.components && !Object.keys(this.config?.components ?? {}).includes(component))) {
            ...
            warnTask({
                text: badComponents.join(', ') + ' components are not installed. Excluding them from the uninstallation...',
            });
        }
        ...
        const components = inputComponents && inputComponents.length > 0 ? { value: inputComponents } : await prompts({
            ...
            choices: Object.entries(this.config?.components ?? {}).map(([name, component]) => ({
                title: `${name}\x1b[0m \x1b[90m${component.version}\x1b[0m`,
                value: name,
            })),
        }, {
            onCancel: () => {
        ...
        const selectedComponents: InputComponents = components.value.map((component: string) => ({
            id: this.project.getComponentId(component),
            version: this.config?.components?.[component]?.version ?? '0.0.0',
            remote: this.config?.components?.[component]?.remote ?? '',
        }));

        await new Components(this.config, this.debug, this.noInput, this.fileNameOverride, this.forceOverride, this.skipPackage, this.dependenciesToKeep).uninstall(selectedComponents);
    }
...
export default new Uninstall().build();
